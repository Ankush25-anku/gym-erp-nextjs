...
const SuperAdminSidebar = ({ pathname }) => {
  ...
  const fetchProfile = async () => {
    try {
      ...
      setProfile({ ...data, role: data.role || "superadmin" });
    } catch (err) {
      console.error("Error fetching profile:", err);
    }
  };

  // Load gym code per user
  useEffect(() => {
    if (
  ...
  const handleSubmitJoin = async () => {
    try {
      ...
      if (res.data.success) {
        ...
        setJoinedGymCode(code);
        localStorage.setItem(`joinedGymCode_${email}`, code);
        setShowJoinGym(false);
        setEnteredCode("");
        setIsValidCode(false);
  ...
  return (
    <>
      ...
      >
        ...
        >
          <img
            src={profile?.profileImage || "/assets/images/profile/profile.jpg"}
            alt="Profile"
            className="rounded-circle"
            style={{ width: "50px", height: "50px", objectFit: "cover" }}
          />
          <span>{profile?.fullName || "Super Admin"}</span>
          {joinedGymCode ? (
            <span
        ...
        <nav className="mt-3 px-2">
          <ul className="sidebar-menu">
            <li>
              <Link href="/superadmin">Dashboard Home</Link>
            </li>
            <li>
              <Link href="/superadmin/users">Users</Link>
            </li>
            <li>
              <Link href="/superadmin/gyms">Gyms</Link>
            </li>
            <li>
              <Link href="/superadmin/addgyms">Add Gym</Link>
            </li>
          </ul>
        </nav>
      </aside>
